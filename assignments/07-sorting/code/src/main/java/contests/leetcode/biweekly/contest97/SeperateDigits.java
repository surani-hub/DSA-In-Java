package contests.leetcode.biweekly.contest97;
...
public class SeperateDigits {
    ...
    static public int[] separateDigits(int[] nums) {
        ...
        for(int num: nums){
            List<Integer> existing = findDigits(num);
            for(int i=existing.size()-1;i>=0;i--){
                digitList.add(existing.get(i));
            }
        }
        return digitList.stream().mapToInt(Integer::intValue).toArray();
    }

    public static List<Integer> findDigits(int num) {
